...
import { ArrowLeft, Play, CheckCircle, RotateCcw, HelpCircle } from 'lucide-react';
...
export const Lesson = () => {
  ...
  const handleRun = async () => {
    if (!lesson) return;
    playSound('click'); // Button handles it, but maybe distinct sound for "Compile"
    // Run the code
    await run(code);
  };

  // Check correctness after execution
  useEffect(() => {
    if (!lesson || isRunning || output.length === 0) return;

    const outputStr = output.join('\n');
    const isCorrect = output.some(line => line.includes(lesson.expectedOutput));
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] md:h-screen md:flex-row bg-intp-base">
      ...
      <div className="w-full md:w-1/3 lg:w-[40%] flex flex-col border-r border-white/5 bg-intp-base/50 backdrop-blur">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <Card className="prose prose-invert prose-sm max-w-none bg-intp-surface/50 border-0">
